...
PROJECTION common/projection.py    savings carried forward on the HAIRCUT record (Sharpe
                                   after both haircuts x vol, demeaned record resampled),
                                   tax by account type. An assumption, printed as one
...
- **eras** — the file that replaced the date picker. `COMMON_ERA_START = '2000-01-01'`, set
  on 2026-09-23 once every published strategy was measurable there: the history extension in
  `common/data_engine.py` (`HISTORY_BACKFILL`) carries each ETF back to 1998-11 on an older
  index fund or a published index, admitted by measurement, and `main.DATA_START_DATE` is
  derived from the era (13 complete months of warm-up before it). It has moved three times
  before — 2008-07, 2001-03, 2004-11 — and the module docstring records why each time. Four **partitions** of that era ship frozen — NBER business cycle, S&P 500
...
`tests/test_anchors.py::TestRegistry` asserts the declaration for all 35 registry keys, plus
...
`main.run_backtest`; Projection tab calls `tools/projection.compute` (the haircut
record carried forward by `common/projection.py`); Live tab calls
`main.compute_live_signals`. Reads and writes
