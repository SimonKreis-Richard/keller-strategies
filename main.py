...
DATA_START_DATE        = '1998-11-01'  # First bar downloaded. DERIVED from the era, not
                                       # chosen: the era opens 2000-01 (common/eras.py), whose
                                       # first return needs a 1999-12 decision, which needs
                                       # LOOKBACK_MONTHS=13 complete months of prices before
                                       # it — 1998-11 through 1999-11. Every donor in
                                       # common/data_engine.HISTORY_BACKFILL reaches that far.
...
STRATEGIES_TO_RUN = [
    ...
    # ---- DM (Antonacci's dual momentum; the four-module composite was deleted 2026-09-23) ----
    # GEMClassic(),
...
TICKERS = [
    ...
    'VEU',
    # (REM, mortgage REITs, left with DM_G8_Composite on 2026-09-23: nothing else read it.)
    # Benchmarks and GTAA
    'VTI', 'IJS', 'EFA',
    # History donors — see common/data_engine.HISTORY_BACKFILL / SYNTHETIC_CASH. These are
    # never traded and never ranked: AGG extends BND, ^IRX builds a T-bill return before BIL
    # existed. EFA and EEM are above because strategies trade them. The mutual funds and
    # indices that carry the era back to 2000 are NOT listed here: the store derives them
    # from HISTORY_BACKFILL, downloads them, and drops them once spliced.
    'AGG', '^IRX',
...
ALL_STRATEGIES = {
    ...
    # DM — Antonacci's dual momentum. `DM_G8_Composite`, his 2012 four-module composite, was
    # DELETED on 2026-09-23 at the owner's decision: its mortgage-REIT leg (REM, 2007-05) is
    # the one product in the registry with no admissible pre-inception substitute, so it alone
    # held the shared window at 2008-07 once everything else reached 2000-01 — and it had never
    # distinguished itself. See the deletion note in strategies/gem.py.
...
# FOURTH registry change, 2026-09-23 (36 -> 35). Deleted: DM_G8_Composite. Not for a defect in
# its rules — those were Antonacci's, verified against SSRN 2042750 — but for the one thing it
# alone could not have: history. The era moved to 2000-01 the same day on mutual-fund and index
# donors (common/data_engine.HISTORY_BACKFILL), and every published entry reached it except
# this one, whose mortgage-REIT leg REM (2007-05) has no admissible donor: the only candidates
# are the mortgage REITs still listed, which lost 9% through the GFC where REM lost 52%,
# because the ones that failed are the ones that vanished. Kept, it would have held the shared
# window at 2008-07 for every other row. The owner judged it had never distinguished itself.
#
...
def compute_live_signals(prices, scores_w, scores_u, strategies, config, broker_accounts,
    ...
    for strat in strategies:
        ...
        # Every refusal that needs the allocation row. Moved to common/live_guards.py on
        # 2026-09-17, unchanged, because the Canadian execution layer produces orders from
        # the same row and a guard living in one caller is a guard the other can forget —
        # which is exactly how these checks took until 2026-07-29 to reach this path at all.
        # `assert_unlevered_defensive` above is the other live guard and necessarily runs
        # earlier, before `generate_allocations` is asked for anything.
        problems = live_guards(strat, alloc, prices, signal_date, store)

        if problems:
            results.append({'name': strat.name, 'stale': stale_note,
                            'error': refusal_message(problems)})
            continue
...
def run_backtest(prices, scores_w, scores_u, strategies, config, store=None):
    ...
    # The decision whose return opens the era: the last month-end BEFORE it. Falls back to
    # the era's first day only for a panel that starts after the era does.
    _before_era = month_ends[month_ends < pd.to_datetime(eras.COMMON_ERA_START)]
    era_floor_decision = (_before_era[-1] if len(_before_era)
                          else pd.to_datetime(eras.COMMON_ERA_START))
    ...
    for strat in strategies:
        ...
        #
        # Clamped to the era's first DECISION, not to its first day. `earliest` is a decision
        # date and the first return lands one period later, so clamping to the 1st of the
        # era's opening month (as this did until 2026-09-23) pushed the first decision to that
        # month's END and the first return a month past the era: every entry measurable
        # before the floor opened one month late, the era's first segment month was measured
        # by nobody, and the report blamed SPY_Benchmark for "starting later". Pinned by
        # tests/test_eras.py::TestTheEraOpensOnItsFirstMonth.
        if cov['earliest'] is not None:
            cov['earliest'] = max(pd.Timestamp(cov['earliest']), era_floor_decision)
