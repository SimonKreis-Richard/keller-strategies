...
>   SPY is **3.18x** (half 1.59x) over the 2000-01 era — it read 4.49x over the 2008-06 window
>   it was first measured on, and that difference is §1.2's point. Separately, §6's claim that
>   *"the canary reduces effective
...
(2000-01…2026-08, two bear markets of about −50% each), full Kelly on SPY is **3.18x** and
half-Kelly **1.59x** — so 2x sits between half and full Kelly, at about two thirds of full.
But `f* = μ/σ²` is **linear in the equity risk premium**, and on a conservative long-run
premium of 5-6% half-Kelly falls to 1.1-1.3x, which puts 2x close to *full* Kelly. Both
readings are the same formula with a different μ — and moving the window's start from 2008-06
to 2000-01, nothing else, cut the measured f\* from 4.49x to 3.18x.
...
or 3x), it is above half Kelly under the realised premium of this window, and it is
...
>
> Measured on this repository's own data, full Kelly on SPY was **4.49x** and half-Kelly
> **2.24x** over 2008-06…2026-06 — roughly 1.7× the figure the whole document rested on — and
> is **3.18x / 1.59x** over the 2000-01 era (§1.1, re-measured 2026-09-23). The correction does not
...
Window **2000-01 … 2026-08, 320 months** — the era (`common/eras.py`), re-measured
2026-09-23. Every row below covers the same months, including the SPY row. Monthly total
returns on real month-end trading dates; `σ` = monthly stdev (ddof=1) × √12, which is
byte-identical to the `vol` column `common/metrics.py::calculate_metrics` prints in the
report; `μ` is net of the **BIL series** (mean 1.84%/yr over this window), not a flat
assumption — realised from 2007-05, and for the 101 months before that constructed from the
13-week bill rate (`^IRX`). Before 2007 most of what the strategy rows hold is constructed
history too ([`KNOWN_GAPS.md`](KNOWN_GAPS.md) §2); SPY's is not.

| Portfolio | μ (excess) | σ | full Kelly f\* | half-Kelly |
|---|---|---|---|---|
| **SPY** (buy & hold) | **7.29%** | **15.15%** | **3.18x** | **1.59x** |
| SPY_Benchmark (as the report prices it) | 7.31% | 15.63% | 2.99x | 1.49x |
| HAA_G12 | 8.47% | 8.69% | 11.22x | 5.61x |
| BAA_G12 | 8.05% | 8.60% | 10.89x | 5.45x |
| DAA_G12 | 7.04% | 8.90% | 8.89x | 4.44x |
| Golden_Butterfly | 6.14% | 8.14% | 9.27x | 4.64x |

**What the start date alone did.** Over the previous window, 2008-06…2026-06 (217 months, then
bound by HYG's 2007-04 inception), the SPY row read **11.15% / 15.76% / 4.49x**. Adding eight
earlier years — the dot-com bear and the whole 2007-09 decline — cut SPY's excess return by a
third and barely moved its volatility. The strategy rows moved much less: HAA_G12's μ is
8.48% → 8.47%, which is the same fact as its return of about +8% through the dot-com bear
(and, for those years, a fact measured partly on donors).

Two rows for SPY because the execution convention moves the answer: buy-and-hold month-end
prices give 3.18x, the registry's `SPY_Benchmark` — same asset, priced through the ledger at
next-open — gives 2.99x. **A 6% spread in f\* from a convention choice alone** (8% over the
2008-06 window) is the first warning that this quantity does not deserve two significant
figures.
...
| 6% | 2.61x | 1.31x |
| 7% | 3.05x | 1.52x |
| **7.29% (measured here)** | **3.18x** | **1.59x** |
| 8% | 3.49x | 1.74x |

**7.29% is not a forecast either; it describes one country's quarter-century.** The window
opens at the top of the dot-com bubble and contains two declines of about 50%, so it is far
less flattering than the 2008-06 window, which opened near the bottom of the financial crisis
and measured 11.15% (f\* 4.49x). That the same formula returned 4.49x and 3.18x depending only
on where the sample starts is this section's argument, measured rather than asserted. And
2000-2026 is still US equity in its strongest era relative to the rest of the world.
If you instead assume a long-run equity risk premium in the 4-6% region — a common range in
the literature, and asserted here rather than derived — then f\* is 1.7-2.6x and half-Kelly is
0.9-1.3x. The old "1.2-1.3x" figure was wrong as a measurement but **it is roughly the right
answer for a 5-6% premium**: it was a defensible assumption presented as a derivation. The
...
Verified on the measured SPY estimates: `g(3.18) = 11.58%`, `g(1.59) = 8.69%`, ratio
...
On the measured window half-Kelly is 1.49-1.59x, so **2x sits above half Kelly — at about two
thirds of full**. (Over the 2008-06 window half-Kelly was 2.07-2.24x and 2x sat just below it;
the eight added years moved it across.) On a long-run 5-6% risk premium half-Kelly is
1.1-1.3x, and **2x is close to full Kelly**. Both readings use the same formula and the same
σ; they differ only in μ.

So the honest position is not *"2x is the closest available instrument to the optimum"* — it
is:

> **2x is above half Kelly even under this window's realised premium, and close to full
> Kelly under any conservative long-run one.** Which of those you believe is a judgement about
> the equity risk premium, not an output of the Kelly criterion.

That is a smaller claim than the one it replaces, the 2000-01 window made it smaller again, and
it is the one the arithmetic supports.
...
> μ_exc 7.29% and σ 15.15% over the 2000-01 era (§1.1), the growth curve reads:
>
> | f | 1.0x | 1.3x | 1.59x | 1.9x | 2.0x | **3.0x** | 3.18x = f\* | 6.35x = 2f\* |
> |---|---|---|---|---|---|---|---|---|
> | g(f) | 6.14% | 7.54% | 8.69% | 9.71% | 9.99% | **11.55%** | 11.58% | 0.00% |
>
> **g(3.0) > g(2.0).** On these estimates 3x is not past the optimum — it sits just below it
> (3.0x against 3.18x; against `SPY_Benchmark`'s 2.99x it is fractionally past), and still
> grows faster than 2x. Over the 2008-06 window (f\* 4.49x) it was comfortably below. The real
> arguments against 3x are the decay term above, the drawdown mathematics in §7, and the fact
> that f\* itself is unreliable (§1.2): at a 5% risk premium f\* is 2.18x and 3x *is* past the
> optimum. **The case against 3x cannot be made
...
growth-optimal leverage is — §1 measures 3.18x on this window (4.49x from 2008-06) and
1.7-2.6x on a conservative risk premium — the set of things you can *buy* as a single
exchange-traded product is fixed:
...
**Re-measured 2026-09-23 over the 2000-01 era** (fills at the next open, 0.10%/side one-way
per leg, cash in BIL, Sharpe/Sortino net of the BIL series, 6%/yr borrow), over
**2000-01 → 2026-08, 320 months** — not a window anybody chose: the floor is the era, which all
three now reach because their tickers' pre-inception years are carried on admitted donors
([`KNOWN_GAPS.md`](KNOWN_GAPS.md) §2), and the run ends at the last complete month in the data.
Until then this table opened 2008-06, held there by HYG's 2007-04 inception, and so began
after the October 2007 peak. It now contains the dot-com bear and the whole of the GFC.
...
The qualitative conclusion **survives a window with two bear markets in it**: signal-following
beats flat margin on CAGR, max drawdown, Sortino and UPI simultaneously, in all three
families, while borrowing less on average. That is not alpha: it is the removal of a cost that
was buying no exposure. The maximum drawdowns did not move for DAA_G12 and BAA_G12 — theirs
were set in the GFC and stay there — and HAA_G12's deepened by 0.05pp.

**The price of margin is unchanged in kind:** against the *unlevered* baseline, 1.3x margin
raises CAGR by **+0.64 to +1.26 pp** (flat: +1.14 / +0.64 / +1.00; signal-following:
+1.26 / +0.88 / +1.05, in the order HAA / DAA / BAA) and worsens max drawdown, Sortino **and**
UPI in **all six levered rows**. Leverage here buys return and sells risk-adjusted quality.
That is a legitimate trade to make deliberately; it is not a free improvement, and the
pre-audit tables — which carried no risk-free rate, no per-leg cost, and no unlevered row —
made it look like one.

*The 2008-06…2026-06 edition of this table (re-measured 2026-07-29) read +0.22 to +1.11 pp on
the same six rows, with the same sign on every comparison. BAA_G12 moved most — 7.34% → 9.95%
unlevered CAGR, Sortino 1.35 → 1.67 — and, for its years before 2007, on donor history
(caveat 4).*
...
2. **Turnover rises with leverage** (see the last column, +12% to +31%): rebalancing a
   levered book trades more notional. This is now charged properly — cost is levied on the executed levered
...
4. **Before 2007 these rows are measured on constructed history.** DAA_G12 and BAA_G12 hold
   high yield, and HYG (2007-04) is carried back on VWEHX, a NAV-smoothed high-yield mutual
   fund which, on the overlap, fell −20% through the GFC where HYG fell −27%. The constructed
   years contain the 2000-2002 bear, so the same direction of bias — flattering the holder in
   a crisis — applies there (KNOWN_GAPS.md §2). The levered *wraps* are not extended at all:
   they reach 2008-03 at best (UWM, 2007-01), and 2010-01 for the G4 sizes (UGL, 2008-12). **A levered strategy's behaviour entering a crash remains the thing this
...
section, and the dashboard carries the same figure as a `Max margin` column. **Re-run
2026-09-23** over the 2000-01 era, with `DM_G8_Composite` gone from the registry; the
multiple-testing haircut reads the full registry's population from `run_facts.json`
(N = 19, trial Sharpe spread 0.112).
...
|---|---|---|---|---|---|---|---|
| HAA_G8_Balanced | **1.296** | margin call | 0.45 | −11.9% | −14.7% | 19.2% | 3.83 |
| HAA_G12 | **1.293** | margin call | 0.45 | −11.2% | −14.9% | 19.1% | 4.40 |
| BAA_G12 | 1.224 | margin call | 0.45 | −10.7% | −14.1% | 20.1% | 6.00 |
| BAA_G1_SPY | 1.205 | margin call | 0.45 | −12.5% | −14.7% | 22.5% | 4.60 |
| HAA_G1_Simple | 1.186 | margin call | 0.45 | −17.1% | −19.8% | 23.6% | 4.05 |
| PAA2_G12 | 1.164 | margin call | 0.45 | −18.2% | −20.5% | 23.8% | 4.81 |
| DAA_G6 | 1.132 | margin call | 0.45 | −19.9% | −20.3% | 25.4% | 1.81 |
| DAA_G12 | 1.128 | margin call | 0.45 | −18.4% | −18.6% | 25.8% | 2.94 |
| VAA_G4 | 1.124 | margin call | 0.45 | −21.8% | −21.8% | 25.8% | 4.12 |
| DAA_G4 | 1.041 | margin call | 0.45 | −21.0% | −21.3% | 30.6% | 1.43 |
| every other 2x/3x holder (17) | **1.000** | margin call / Kelly | 0.90–0.99 | −18 to −82% | −27 to −85% | 45–99% | 0.58–2.55 |
| VAA_G12 | 1.000 | margin call | 0.45 | −28.0% | −29.4% | 38.4% | 1.34 |
| BAA_G4 | 1.000 | margin call | 0.45 | −19.1% | −29.0% | 39.9% | 3.38 |
| GEM_G2_Classic | 1.000 | margin call | 0.45 | −22.9% | −33.7% | 51.2% | 1.45 |
| GTAA_G5 | 1.000 | **Kelly gate** | 0.45 | −13.3% | −13.8% | 18.8% | 0.59 |
| Golden_Butterfly | 1.000 | **Kelly gate** | 0.45 | −16.9% | −21.1% | 29.5% | 0.42 |
| Sixty_Forty_1X | 1.000 | **Kelly gate** | 0.45 | −29.7% | −32.6% | 40.4% | −0.84 |
| SPY_Benchmark | 1.000 | **Kelly gate** | 0.45 | −51.3% | −55.2% | 69.0% | −0.05 |
| DM_G5_Leveraged_3X | 1.000 | **Kelly gate** | 0.99 | −81.8% | −84.4% | 100.0% | −0.03 |

*"Every other 2x/3x holder" is the fourteen leveraged wraps other than `DM_G5_Leveraged_3X`
plus the three levered benchmarks; each is capped at 1.000, eight of them by the Kelly gate.
The wraps and levered benchmarks are measured over their own products' history, not the era.*
...
|---|---|---|
| 5 — prudent | **2** | HAA_G12 at 1.03x |
| 3 — balanced | 10 | HAA_G8_Balanced at 1.30x (HAA_G12 1.29x) |
| 2 — aggressive | 14 | HAA_G8_Balanced at 1.49x (HAA_G12 1.49x) |

At `k=5` the registry supports essentially no margin: two entries clear 1.00x, by 2-3%. (Over
the 2008-06 window the same sweep read 0 / 11 / 13, best 1.27x and 1.47x.) That is the honest
shape of the answer: the recommendation is not a number this data pins down to two decimals,
it is a band from 1.0x to about 1.5x whose position is set by how many times the stressed
drawdown you insist on surviving. Nothing about a broker moves it nearly as much.

Three things fall out of that table, and none of them is a preference:

1. **At `k=3`, nothing in the registry supports meaningful margin.** The best entries are 1.30x
   and 1.29x, and they are the two with the smallest stressed drawdown in the repository. `k=3`
   against a 19% stressed drawdown demands surviving a 57% decline, and at `m=0.45` that is
   worth about 1.3x. Anyone expecting 1.3x to be comfortably available should read those rows
   first.
2. **The intra-period correction is large, and it is real.** *Every* row's daily-path drawdown
   ...
   the worst of it) to **+55.0%** (`HAA_G4_Leveraged_2X`, −22.8% on month ends against −35.3%
   on the daily path of the same held allocation). `HAA_G12` sits at +32.7% (−11.2% against
   −14.9%). `DAA_G3_Leveraged_3X` posts the deepest absolute gap, −47.8% against **−65.4%**,
   and among the 2x wraps `HAA_G3_Leveraged_2X`, −37.4% against −51.7%. That gap is invisible
   everywhere else in this repository, because `run_ledger` never looks inside a month.
3. **Twelve entries fail the Kelly gate outright** at a 4.15pp borrow spread — `Golden_Butterfly`,
   the 60/40, `GTAA_G5`, `SPY_Benchmark`, and eight levered entries (the three DM wraps, four
   other 3x wraps and `SPY_2X_Benchmark`). `GEM_G2_Classic` now passes the gate and is stopped
   by its margin call instead. A low-volatility portfolio cannot pay 4.15pp on borrowed money; the gate is not
...
> withdrawn 1.2-1.3x figure, and the first is **backwards**: measured full Kelly on SPY was
> 4.49x over the 2008-06 window and is 3.18x over the 2000-01 era, so 3x is *at or just below*
> it, not far beyond it (§2's growth table still shows g(3.0) > g(2.0), now narrowly). 3x is declined here because its decay term is three times 2x's, because §7's
...
Measured over 2000-01…2026-08 (320 months; re-measured 2026-09-23, and the 2008-06 edition
read the same way in every row), splitting each family's own months by whether its signal was
**fully offensive** in the month that earned the return:

| family | months fully offensive | σ, all months | σ, offensive months only | |
|---|---|---|---|---|
| HAA_G12 | 256 / 320 | 8.69% | **9.30%** | higher |
| DAA_G12 | 168 / 320 | 8.90% | **10.26%** | higher |
| BAA_G12 | 140 / 320 | 8.60% | **9.58%** | higher |

**The canary does not truncate the volatility of the invested state. It dilutes the
unconditional figure by spending a fifth (HAA) to more than half (BAA) of all months at least
partly in defensive assets.** Those are different things, and only the first would justify levering harder:
...
|---|---|---|---|---|
| HAA_G12 | 11.22x | 10.97x | 9.48% | 9.30% |
| DAA_G12 | 8.89x | **9.56x** | 10.07% | 10.26% |
| BAA_G12 | 10.89x | **13.59x** | 12.46% | 9.58% |
...
3.18x, every one of these is three to four times larger.
...
selected, in part, for having done well over this exact window — and, for its years before
2007, measured on donor history rather than on the funds themselves.
...
- The unlevered families' maximum drawdowns run −10.7% to −18.4% against SPY's **−50.8%**
  (month-end, buy and hold; −51.3% as the report prices it) over the same 2000-01 window.
...
1. Kelly gives the growth-optimal leverage as f* = μ/σ², which is LINEAR in the equity
   risk premium — measured 3.18x on SPY here (4.49x from 2008-06), 1.7-2.6x on a
   conservative 4-6% premium.
