...
@dataclass(frozen=True)
class SharpeHaircut:
    """The two haircuts and the one that is used. NaN where a haircut could not be computed."""

    observed: float
    deflated: float          # SR - E[max SR]: the selection haircut
    lower_bound: float       # SR - z*SE: the estimation haircut
    used: float              # the more severe of the two that exist
    dsr: float               # P(true SR > 0), reported, never gating
    #: Why a haircut is missing. The first kind makes the result optimistic, so it is an
    #: ASSUMPTION the reader must see; the second only narrows what was computed.
    assumptions: tuple = ()
    notes: tuple = ()


def haircut_sharpe(sharpe_annual, returns, n_trials, trial_sharpe_sd, sharpe_z=1.0,
                   selected=True):
    """`SR_used = min(SR - E[max SR], SR - z*SE)`, and why either half is missing.

    One definition, two consumers: the Kelly gate in `recommend_leverage` and the expected
    return in `common/projection.py`. A second copy of the chain would be a second answer to
    "how much of that Sharpe is selection".

    `selected=False` skips the multiple-testing term, for an entry nobody chose from a list
    (a passive benchmark). The estimation haircut still applies: a benchmark's Sharpe is
    measured on the same finite sample as everything else.
    """
    r = pd.Series(returns).dropna()
    sr_obs = float(sharpe_annual)
    sr_defl = sr_lower = dsr = float('nan')
    assumptions, notes = [], []
    if selected:
        try:
            sr_defl = sr_obs - expected_max_sharpe(n_trials, trial_sharpe_sd)
            dsr = deflated_sharpe_probability(sr_obs, r, n_trials, trial_sharpe_sd)
        except NotCalculable as exc:
            assumptions.append(f'No multiple-testing haircut applied: {exc}. The reported '
                               'Sharpe is therefore the selected one, and selection makes it '
                               'optimistic.')
    try:
        sr_lower = sr_obs - float(sharpe_z) * sharpe_standard_error(sr_obs, r)
    except NotCalculable as exc:
        notes.append(f'no Sharpe standard error: {exc}')
    candidates = [s for s in (sr_defl, sr_lower) if not math.isnan(s)]
    sr_used = min(candidates) if candidates else float('nan')   # the more severe of the two
    return SharpeHaircut(sr_obs, sr_defl, sr_lower, sr_used, dsr,
                         tuple(assumptions), tuple(notes))
...
def recommend_leverage(kpis, policy):
    ...
    cut = haircut_sharpe(kpis.sharpe, r, policy.n_trials, policy.trial_sharpe_sd,
                         policy.sharpe_z)
    sr_obs, sr_defl, sr_lower, sr_used, dsr = (cut.observed, cut.deflated, cut.lower_bound,
                                               cut.used, cut.dsr)
    assumptions.extend(cut.assumptions)
    notes.extend(cut.notes)
