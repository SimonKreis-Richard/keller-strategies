...
class TestTheTableReadsAsNumbers(unittest.TestCase):
    """Alignment and tints. Presentation only — but a tint pointing the wrong way would paint
    the deepest drawdown green, which is a misstatement rather than a style."""

    def _table(self, metrics):
        head, regime = ui.column(), ui.column()
        app._render_ranked_block(head, metrics, 'sortino', advice=None, regime_container=regime)
        return next(el for el in head.default_slot.children if type(el).__name__ == 'Table')

    def test_numeric_columns_are_right_aligned_and_names_are_not(self):
        metrics, _ = _fixture()
        align = {c['name']: c['align'] for c in self._table(metrics).columns}
        for col in ('Rank', 'CAGR', 'Max Drawdown', 'Sharpe', 'Sortino', 'Volatility'):
            self.assertEqual(align[col], 'right', col)
        for col in ('Strategy', 'Measured', 'Coverage'):
            self.assertEqual(align[col], 'left', col)

    def test_the_shallowest_drawdown_is_green_and_the_deepest_red(self):
        metrics, _ = _fixture()
        for d, dd in zip(metrics, (-0.10, -0.20, -0.40)):
            d['max_dd'] = dd
        rows = {r['Strategy']: r for r in self._table(metrics).rows}
        self.assertIn('0,158,115', rows['AAA']['_tint']['Max Drawdown'])   # -10%: green
        self.assertIn('213,94,0', rows['CCC']['_tint']['Max Drawdown'])    # -40%: red

    def test_a_higher_sortino_is_green(self):
        metrics, _ = _fixture()                     # Sortino AAA 1.4 > CCC 1.1 > BBB 0.9
        rows = {r['Strategy']: r for r in self._table(metrics).rows}
        self.assertIn('0,158,115', rows['AAA']['_tint']['Sortino'])
        self.assertIn('213,94,0', rows['BBB']['_tint']['Sortino'])

    def test_a_column_with_no_spread_is_not_tinted(self):
        metrics, _ = _fixture()                     # every fixture row has Sharpe 0.8
        for row in self._table(metrics).rows:
            self.assertNotIn('Sharpe', row['_tint'])

    def test_tints_add_no_column(self):
        metrics, _ = _fixture()
        self.assertNotIn('_tint', [c['name'] for c in self._table(metrics).columns])

    def test_the_numeric_test_ignores_placeholders_but_not_words(self):
        self.assertTrue(app._is_numeric_column([{'x': '1.50x'}, {'x': '—'}, {'x': '-8.42%'}], 'x'))
        self.assertFalse(app._is_numeric_column([{'x': '2008-07..2026-06'}], 'x'))
        self.assertFalse(app._is_numeric_column([{'x': '—'}, {'x': 'n/a'}], 'x'))
...
class TestTheNamedCrisesArePresent(unittest.TestCase):
    ...
    def test_the_episode_panel_names_the_conventional_crises(self):
        ...
        for label in ('Dot-com bust', 'Global financial crisis', 'COVID crash',
                      'Inflation / rate bear'):
...
class TestTheProjectionTab(unittest.TestCase):
    """Wiring only: the numbers are tested in `tests/test_projection.py`."""

    def _projection(self):
        from common import projection as proj
        rf = pd.Series(0.0015, index=_returns().index)
        entries = []
        for name, role, seed in (('AAA', 'strategy', 1), ('SPY_Benchmark', 'benchmark', 2)):
            e = _entry(name, seed)
            e.update({'role': role, 'rf_series': rf})
            entries.append(e)
        accounts = [proj.AccountPlan('FICTIONAL', 'CELI', 1000.0, 100.0)]
        tax = proj.TaxAssumptions(0.3, 0.3)
        return proj.project(entries, accounts, tax, 6, 0.02, 19, 0.112, 'test', n_paths=100,
                            seed=3)

    def test_the_tab_exists_between_backtest_and_live(self):
        labels = [el.props.get('label') for el in app.tabs.default_slot.children]
        self.assertEqual(labels, ['Backtest', 'Projection', 'Live signals'])

    def test_the_render_says_what_it_is_before_it_draws(self):
        col = ui.column()
        app._render_projection(col, self._projection(), demo=True)
        texts = _texts(col)
        self.assertIn('not a forecast', texts[0])
        self.assertTrue(any('DEMONSTRATION' in t for t in texts))
        self.assertTrue(any('used' in t for t in texts), 'the haircut is not stated')
        self.assertEqual(_charts(col), 1)
        table = _all_tables(col)[0]
        self.assertIn('savings account', {r['Line'] for r in table.rows})
        self.assertIn('P(< put in)', [c['name'] for c in table.columns])

    def test_the_inputs_make_a_block_the_cli_accepts(self):
        from common import projection as proj
        saved = (app.state['accounts'], app.state['projection'])
        try:
            app.state['accounts'] = [{'account_name': 'FICTIONAL', 'account_balance': 1.0,
                                      'account_priority': 1}]
            app.state['projection'] = {'tax': {}, 'accounts': {
                'FICTIONAL': {'kind': 'REER', 'monthly_contribution': 50.0,
                              'contribution_growth': 0.0, 'annual_cap': None,
                              'reinvest_refund': False},
                'GONE': {'kind': 'CELI'}}}
            block = app._projection_block()
            self.assertNotIn('GONE', block['accounts'])
            _settings, accounts, _tax = proj.parse_config(
                {'BROKER_ACCOUNTS': app.state['accounts'], 'PROJECTION': block})
            self.assertEqual(accounts[0].kind, 'REER')
            self.assertFalse(accounts[0].reinvest_refund)
        finally:
            app.state['accounts'], app.state['projection'] = saved

    def test_an_account_with_no_kind_is_refused_by_name(self):
        from common import projection as proj
        cfg = {'BROKER_ACCOUNTS': [{'account_name': 'FICTIONAL', 'account_balance': 1.0}],
               'PROJECTION': {'strategy': 'AAA', 'comparator': None, 'horizon_years': 5,
                              'inflation': 0.02, 'currency': 'CAD',
                              'tax': {'marginal_rate_now': 0.3, 'marginal_rate_retirement': 0.3},
                              'accounts': {'FICTIONAL': {'monthly_contribution': 1.0}}}}
        with self.assertRaisesRegex(proj.ConfigError, 'FICTIONAL.*no kind chosen'):
