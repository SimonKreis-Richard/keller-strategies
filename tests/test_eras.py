...
class TestTheEraOpensOnItsFirstMonth(unittest.TestCase):
    """An entry measurable BEFORE the era must earn its first return IN the era's first month.

    Found 2026-09-23. `run_backtest` clamped each entry's first decision to the 1st of the
    era's opening month, and a decision is a month-END, so the first decision became that
    month's end and the first return landed a month late: with the era at 2004-11,
    SPY_Benchmark and Sixty_Forty_1X both opened 2004-12, the 2004-11 column was measured by
    nobody, and the report said the window was "set by SPY_Benchmark, Sixty_Forty_1X, whose
    assets start later" — assets that were in fact measurable from 2001.

    Behavioural rather than a source check: the era floor is patched to a date inside the
    synthetic store, where SPY is measurable 13 months earlier, and the first return must fall
    in the floor's own month — for the ranked row AND for the full-history row the regime
    panel reads.
    """
    ...
    def setUpClass(cls):
        ...
        store = make_daily_store(n_days=1400, start='2013-01-01')
        config = {
            'START_DATE': cls.FLOOR, 'END_DATE': None,
            'DATA_START_DATE': '2013-01-01', 'EXECUTION_MODE': False,
            'CURRENT_EXECUTION_DATE': None,
            'LEVERAGE_FACTOR': 1.0, 'MARGIN_BORROW_RATE': 0.06,
            'MARGIN_FOLLOWS_SIGNAL': True, 'COST_PCT_PER_SIDE': 0.001,
        ...
        prices, s_w, s_u = main.build_signal_panel(store, config)
        with patch.object(main.eras, 'COMMON_ERA_START', cls.FLOOR):
            cls.metrics, _ = main.run_backtest(
                prices, s_w, s_u, [main.ALL_STRATEGIES['SPY_Benchmark']()], config,
                store=store)

    def test_the_first_ranked_return_is_in_the_eras_first_month(self):
        d = self.metrics[0]
        self.assertEqual(pd.Period(d['first_return'], 'M'), pd.Period(self.FLOOR, 'M'),
                         'an entry measurable before the era opened a month late')

    def test_the_full_history_row_opens_there_too(self):
        d = self.metrics[0]
        self.assertEqual(pd.Period(d['first_return_full'], 'M'), pd.Period(self.FLOOR, 'M'))

    def test_nobody_is_blamed_when_the_floor_binds(self):
        self.assertEqual(self.metrics[0]['window_binding'], (),
                         'the era floor bound the window, yet a strategy was named as '
                         'starting later')
