...
class TestRegimePanelIgnoresTheRequestedWindow(unittest.TestCase):
    ...
    def test_but_it_stops_at_the_ERA_floor_which_is_not_a_request(self):
        ...
        so its `earliest` can predate the era. `run_backtest` clamps to the era floor for that
        reason — and clamping to `start_floor` instead would look identical in production (they
        are the same object there) while making the panel inherit the caller's dates, which
        the three tests around this one exist to forbid. It was written the wrong way first,
        and they caught it.

        Asserted on the SOURCE because in production the two are the same object. Since
        2026-09-23 the clamp reads `era_floor_decision` — the decision BEFORE the era opens,
        so the era's first month is measured (see tests/test_eras.py::
        TestTheEraOpensOnItsFirstMonth) — and that must itself be derived from
        `eras.COMMON_ERA_START`, never from the request.
        """
        import inspect
        ...
        self.assertIn('era_floor_decision', window,
        ...
        derivation = [ln for ln in src.splitlines() if ln.strip().startswith('_before_era =')]
        self.assertEqual(len(derivation), 1)
        self.assertIn('eras.COMMON_ERA_START', derivation[0])
        self.assertNotIn('start_floor', derivation[0])
